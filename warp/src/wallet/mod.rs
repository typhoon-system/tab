...
pub type WalletEventStream = BoxStream<'static, WalletEventKind>;
...
dyn_clone::clone_trait_object!(Wallet);
...
#[async_trait::async_trait]
pub trait WalletStream: Sync + Send {
    ...
    async fn raygun_subscribe(&mut self) -> Result<WalletEventStream, Error> {
        Err(Error::Unimplemented)
    }
}

#[async_trait::async_trait]
pub trait WalletEvents: Sync + Send {
